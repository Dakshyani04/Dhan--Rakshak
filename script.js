    ...
    new Chart(barCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: colors, // Use dynamic colors
                borderWidth: 1
            }]
        },
        options: {
...
// Function to generate darker dynamic colors for the bar chart
function generateColors(count) {
    const colors = [];
    for (let i = 0; i < count; i++) {
        // Generate darker colors by limiting RGB values to a lower range
        ...
        const color = `rgba(${r}, ${g}, ${b}, 0.8)`; // Use 0.8 for slightly transparent darker colors
        colors.push(color);
    }
    return colors;
}
...
function handleOpenPdfButtonClick() {
    const bookPdfLink = document.getElementById('book-pdf-link');
    if (bookPdfLink && bookPdfLink.href) {
        window.open(bookPdfLink.href, '_blank'); // Open the PDF in a new tab
    } else {
...
if (openPdfButton) {
    openPdfButton.addEventListener('click', handleOpenPdfButtonClick);
}
...
const protectedPages = ["Dashboard.html", "expense-history.html", "expense-analytics.html", "book-recommendation.html", "passive-income.html", "download-log.html"];
const currentPage = window.location.pathname.split('/').pop();
